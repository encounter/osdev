...
void __ubsan_handle_pointer_overflow(void *data_raw,
    ...
    ubsan_value_handle_t base = (ubsan_value_handle_t) lhs_raw;
    ubsan_value_handle_t result = (ubsan_value_handle_t) rhs_raw;
    const char* violation = "pointer index expression overflowed";
    if ((base >= 0) == (result >= 0)) {
        if (base > result) {
            violation = "addition of unsigned offset overflowed";
        } else {
            violation = "subtraction of unsigned offset overflowed";
        }
    }
